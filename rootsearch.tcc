...
Move Game::rootSearch(unsigned int endDepth) {
    ...
    if (nMoves <= 1) {
        ...
        alpha0.v = -infinity*C;
        T<(Colors)-C> beta;
        beta.v = infinity*C; //both alpha and beta are lower limits, viewed from th color to move
        Score<C> value;
        ...
        for (depth=eval.dMaxExt+2; depth<=endDepth; depth++) {
            ...
            if (alpha0.v != -infinity*C && abs(alpha0.v) < 1024) {
                ...
                if (value <= alpha2.v) {
                    ...
                    if (!infinite && now > softLimit && alpha > alpha0.v + C*eval.evalHardBudget)
                        break;
                    T<(Colors)-C> beta3;
            ...
            now = system_clock::now();
            if (Options::noisy || Options::humanreadable) console->send(status(now, alpha.v));
...
//                 T<C> oldalpha(alpha);
                ...
                if (!infinite) {
                    ...
                    if (now > softLimit && alpha > alpha0.v + C*eval.evalHardBudget)
                        searchState = Stopping; } } // for ml
            //         now = system_clock::now();
            ...
            if (alpha <= -infinity*C) break;
            if (searchState) break;
            alpha0.v = alpha.v; } // for depth
    } // if nMoves
    commonStatus();
//    if (bestScore > infinity*C)
//        mateFinder<C>(ml, softLimit, b);
...
template<Colors C>
void Game::mateFinder(SortedMoveList& ml, CHRONO::system_clock::time_point softLimit, const ColoredBoard<C>& b) {
    ml.begin();
    //             ml.sort(bestMovesLimit);
    bestMove = currentMove = *ml;
    currentMoveIndex = 1;
    ...
    #endif
    Score<C> alpha;
    const Score<(Colors)-C> beta;
    Score<C> value(bestScore*C);
    ...
    if (!infinite && now > softLimit) return;
    ...
    for (++ml; ml.isValid() && searchState == Running; ++ml) {
        ...
        ColoredBoard<(Colors)-C> nextboard(b, *ml, *this);
        bool doNull = false;
        value.v = search9<(Colors)-C,trunk>(doNull, 0, nextboard, depth-1, beta, alpha, ExtNot, NodeFailHigh NODE );
        if (searchState) break;
        ml.nodesCount(WorkThread::stats.node - subnode);
        if (value <= alpha.v) continue;
        alpha.v = value.v;
