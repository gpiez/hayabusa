...
Move RootBoard::rootSearch(unsigned int endDepth) {
    ...
    search<(Colors)-C, tree>(b, *ml, dMaxCapture+dMaxThreat, beta, alpha0, ply+1, ExtNot, maxDepth NODE);
    ml.nodesCount(stats.node - subnode);
    for (++ml; ml.isValid() && !stopSearch; ++ml) {
        uint64_t subnode = stats.node;
        if (search<(Colors)-C, tree>(b, *ml, dMaxCapture+dMaxThreat, beta, alpha0, ply+1, ExtNot, maxDepth NODE)) {
            bestMove = *ml;
            ml.nodesCount(stats.node - subnode);
            ml.currentToFront();            
        }
